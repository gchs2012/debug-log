...
static int ss_add_event(ss_event_loop *eventLoop, int fd, int mask)
{
    ...
    if (epoll_ctl(eventLoop->epfd, op, fd, &ee) < 0) {
        DG_LOG(EP_LOG, "epoll_ctl error: %s\n", strerror(errno));
        return -1;
    }
...
static void ss_del_event(ss_event_loop *eventLoop, int fd, int mask)
{
    ...
    ss_event *se = &eventLoop->events[fd];
    SS_RETURN(se->mask == SS_NONE);
    ...
    se->mask = se->mask & (~mask);
    if (fd == eventLoop->maxfd && se->mask == SS_NONE) {
        /* Update the max fd */
        int j;
    ...
    ss_event_proc proc, void *clientData)
{
    ...
    ss_event *ev = &eventLoop->events[fd];
    SS_RETURN_RES(ss_add_event(eventLoop, fd, mask) < 0, -1);

    ev->mask |= mask;
    if (mask & SS_READABLE) ev->reventProc = proc;
    if (mask & SS_WRITABLE) ev->weventProc = proc;
...
static int ss_set_reuse_addr(int fd)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) {
        DG_LOG(EP_LOG, "setsockopt SO_REUSEADDR error: %s\n", strerror(errno));
        return -1;
    }
...
static int ss_create_socket(void)
{
    ...
    if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
        DG_LOG(EP_LOG, "create socket error: %s", strerror(errno));
        return -1;
    }
...
{
    if (bind(fd, sa, len) < 0) {
        DG_LOG(EP_LOG, "bind socket error: %s\n", strerror(errno));
        return -1;
    }

    if (listen(fd, 512) < 0) {
        DG_LOG(EP_LOG, "listen socket error: %s\n", strerror(errno));
        return -1;
    }
...
static int ss_set_block(int fd, int non_block)
{
    ...
    if (fcntl(fd, F_SETFL, flags) == -1) {
        DG_LOG(EP_LOG, "fcntl(F_SETFL, O_NONBLOCK): %s\n", strerror(errno));
        return -1;
    }
    ...
    int fd, int mask, void *privdata)
{
    ...
    if (len < 0) {
        if (errno == EAGAIN) {
            ...
            DG_LOG(EP_LOG, "reading from client error: %s\n", strerror(errno));
    ...
    int fd, int mask, void *privdata)
{
    ...
    if (len < 0) {
        if (errno == EAGAIN) {
            ...
            DG_LOG(EP_LOG, "writing to client error: %s\n", strerror(errno));
            ss_close_client(eventLoop, c);
        }
    } else if (len == 0) {
        ss_close_client(eventLoop, c);
    ...
    int fd, int mask, void *privdata)
{
    ...
    cfd = ss_accept(fd);
    SS_GOTO_LABLE((cfd < 0) || (cfd >= eventLoop->setsize), err);

    c = &eventLoop->client[cfd];
    ss_set_block(cfd, 1);
    ret = ss_create_event(eventLoop, cfd, SS_READABLE, ss_read_from_client, c);
...
void ss_process_events(void)
{
    ...
    for (i = 0; i < numevents; i++) {
        ss_event *ev = &eventLoop->events[eventLoop->fired[i].fd];
        int mask = eventLoop->fired[i].mask;
        int fd = eventLoop->fired[i].fd;
        int rfired = 0;

        /* 调用read回调函数 */
        if (ev->mask & mask & SS_READABLE) {
            rfired = 1;
            ev->reventProc(eventLoop, fd, mask, ev->clientdata);
        }
        /* 调用write回调函数 */
        if (ev->mask & mask & SS_WRITABLE) {
            if (!rfired || (ev->weventProc != ev->reventProc)) {
                ev->weventProc(eventLoop, fd, mask, ev->clientdata);
            }
        }
    }
}
...
/*****************************************************************************
    ...
    日    期 : 2018年8月2日
*****************************************************************************/
int ss_init_server(int num, int log_num)
{
    int ret;
    int fd = -1;
    int epoll_fd = -1;
    ...
    if (epoll_fd < 0) {
        DG_LOG(EP_LOG, "epoll create error: %s", strerror(errno));
        return -1;
    }
    ...
    snprintf(sa.sun_path, sizeof(sa.sun_path), SS_UNIX_SOCKET, num);
    unlink(sa.sun_path);
